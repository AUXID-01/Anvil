...
class Engine(Adapter):
    def __init__(self,
        ...
        # Empirical geometry probe mode switch
        # Valid modes: "identity", "ramp", "exp", "random", "eig_min", "eig_max", "spectral", "slow_spectral"
        self.geometry_mode = "slow_spectral"
    ...
    def predict_precision(self, corrupted_query: np.ndarray) -> np.ndarray:
        ...
        # 2. Geometry Branch: Slow Spectral steering
        if self.geometry_mode == "identity":
            geom = np.ones(self.N)
        elif self.geometry_mode == "slow_spectral":
            geom = self.precomputed_patterns[k1]["slow_geom"]
        elif self.geometry_mode == "spectral":
            geom = self.precomputed_patterns[k1]["spectral_geom"]
        else:
            # Fallback for other modes implemented in probe phase
            geom = np.ones(self.N)

        # 3. Apply geometry multiplicatively with Critical Damping
